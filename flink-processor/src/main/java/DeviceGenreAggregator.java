...
public class DeviceGenreAggregator {
    public static void aggregateViewsByDeviceAndGenre(DataStream<Event> eventsStream) {
        KeyedStream<Tuple3<String, String, Integer>, Tuple2<String, String>> keyedStream = eventsStream
            .map(event -> new Tuple3<>(event.DeviceType, event.Genre, 1))
        ...
        DataStream<Tuple4<String, String, Integer, String>> viewsByDeviceGenre = keyedStream
        ...
        viewsByDeviceGenre.addSink(JdbcSink.sink(
            "insert into viewsByDeviceGenreAggregation (device_type, genre, view_count, window_end_utctime) values (?, ?, ?, ?)",
            (statement, tuple) -> {
