...
public class LocationMRAggregator {
    public static void aggregateViewsByLocationAndMR(DataStream<Event> eventsStream) {
        KeyedStream<Tuple3<String, String, Integer>, Tuple2<String, String>> keyedStream = eventsStream
            .map(event -> new Tuple3<>(event.UserLocation, event.MaturityRating, 1))
        ...
        DataStream<Tuple4<String, String, Integer, String>> viewsByLocationMR = keyedStream
            ...
            viewsByLocationMR.addSink(JdbcSink.sink(
            "insert into viewsByLocationMRAggregation (country, maturity_rating, view_count, window_end_utctime) values (?, ?, ?, ?)",
            (statement, tuple) -> {
