...
from pyspark.sql.functions import col, to_date, hour, minute, StringType
from google.cloud import bigquery

# Use environment variable for BigQuery project name
gcp_project_name = os.getenv('GCP_PROJECT')
...
def load_data_to_bigquery():
    ...
    create_bq_table_with_sql(f"{gcp_project_name}.{bigquery_dataset}.{bigquery_table_1}", schema_query1)
    create_bq_table_with_sql(f"{gcp_project_name}.{bigquery_dataset}.{bigquery_table_2}", schema_query2)
    create_bq_table_with_sql(f"{gcp_project_name}.{bigquery_dataset}.{bigquery_table_3}", schema_query3)
    ...
    table_id1 = f"{gcp_project_name}.{bigquery_dataset}.{bigquery_table_1}"
    ...
    table_id2 = f"{gcp_project_name}.{bigquery_dataset}.{bigquery_table_2}"
    ...
    table_id3 = f"{gcp_project_name}.{bigquery_dataset}.{bigquery_table_3}"
